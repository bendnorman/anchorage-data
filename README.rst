Anchorage Data: a Python Template Repository for Catalyst
=======================================================================================

.. readme-intro

.. image:: https://github.com/catalyst-cooperative/ancdata/workflows/tox-pytest/badge.svg
   :target: https://github.com/catalyst-cooperative/ancdata/actions?query=workflow%3Atox-pytest
   :alt: Tox-PyTest Status

.. image:: https://github.com/catalyst-cooperative/ancdata/workflows/repo2docker/badge.svg
   :target: https://github.com/catalyst-cooperative/ancdata/actions?query=workflow%3Arepo2docker
   :alt: repo2docker Build Status

.. image:: https://github.com/catalyst-cooperative/ancdata/workflows/docker-build-push/badge.svg
   :target: https://github.com/catalyst-cooperative/ancdata/actions?query=workflow%3Adocker-build-push
   :alt: Docker build status

.. image:: https://img.shields.io/codecov/c/github/catalyst-cooperative/ancdata?style=flat&logo=codecov
   :target: https://codecov.io/gh/catalyst-cooperative/ancdata
   :alt: Codecov Test Coverage

.. image:: https://img.shields.io/readthedocs/catalystcoop-ancdata?style=flat&logo=readthedocs
   :target: https://catalystcoop-ancdata.readthedocs.io/en/latest/
   :alt: Read the Docs Build Status

.. image:: https://img.shields.io/pypi/v/catalystcoop.ancdata?style=flat&logo=python
   :target: https://pypi.org/project/catalystcoop.ancdata/
   :alt: PyPI Latest Version

.. image:: https://img.shields.io/conda/vn/conda-forge/catalystcoop.ancdata?style=flat&logo=condaforge
   :target: https://anaconda.org/conda-forge/catalystcoop.ancdata
   :alt: conda-forge Version

.. image:: https://img.shields.io/pypi/pyversions/catalystcoop.ancdata?style=flat&logo=python
   :target: https://pypi.org/project/catalystcoop.ancdata/
...
``ancdata`` (the cat who isn't entirely there...).
...
* The name of the repository should be the same as the name of the new Python package
  you are going to create. E.g. a repository at ``catalyst-cooperative/ancdata`` should
  be used to define a package named ``ancdata``.
...
* Create the ``ancdata`` conda environment by running ``conda env create`` or
  (preferably) ``mamba env create`` in the top level of the repository.
* Activate the new conda environment with ``conda activate ancdata``.
...
Once you know that your forked version of the ``ancdata`` package is working as
...
Python Package Index (PyPI) or on ``conda-forge``. Similarly the ``ancdata`` package
becomes the ``catalystcoop.ancdata`` distribution. The distribution name is determined
by the ``name`` argument in the call to ``setup()`` in ``setup.py``.

.. code:: bash

  pip install catalystcoop.ancdata
...
use ``grep -r`` to search recursively through all of the files for the word ``ancdata``
...
* Rename the ``src/ancdata`` directory to reflect the new package name.
* Search for ``ancdata`` and replace it as appropriate everywhere. Sometimes
  this will be with a distribution name like ``catalystcoop.ancdata``
  (the package as it appears for ``pip`` or ``PyPI``) and sometimes this will be the
  importable package name (the name of the directory under ``src`` e.g. ``ancdata``)
...
* When the tests are run via the ``tox-pytest`` workflow in GitHub Actions, the test
  ...
  the results
  `for the ancdata repo <https://app.codecov.io/gh/catalyst-cooperative/ancdata>`__.
...
* It will ask you for a project name -- this will become part of the domain name for the
  ...
  dots and underscores replaced with dashes. E.g. ``catalystcoop-ancdata`` or
